...
from DES_weather_analysis import battery, boilers, solar_PV, CHP_system, wind_turbine
class TwoStageOpt(Problem):
    def __init__(self,path_test,output_prefix):
        self.path_test = path_test
        components_path = os.path.join(path_test,'Energy Components')
        self.editable_data_path =os.path.join(path_test, 'editable_values_design.csv')
        ...
        renewable_percentage = float(self.editable_data['renewable percentage'])  #Amount of renewables at the U (100% --> 1,mix of 43% grid--> 0.463, mix of 29% grid--> 0.29, 100% renewables -->0)
        self.electricity_EF =  int(self.editable_data['city EF'])*renewable_percentage*lbstokg_convert/100
        ...
        self.representative_days_path =  os.path.join(path_test,'ScenarioReduction')
    ...
    def evaluate(self, solution):
        represent_day_max_results = self.represent_day_max(self.output_prefix)
        if self.use_solar_PV=='yes':
        ...
        if self.use_battery=='yes':
            ...
            self.CAP_battery = 0
        operating_cost_initialized = self.operating_cost(self.A_solar,self.A_swept,self.CAP_CHP_elect,self.CAP_boiler,self.CAP_battery,self.output_prefix)
    ...
    def operating_cost(self,_A_solar,_A_swept,_CAP_CHP_elect,_CAP_boiler,_CAP_battery,output_prefix):
        ...
        for represent in range(self.num_clusters):
            E_bat = {}
            representative_day[represent] = pd.read_csv(os.path.join(self.representative_days_path,output_prefix+'Represent_days_modified_'+str(represent)+'.csv'))
            self.G_T = list(representative_day[represent]['gti']) #Global Tilted Irradiation (Wh/m^2) in Slat Lake City from TMY3 file for 8760 hr a year on a titled surface 35 deg
            self.V_wind = list(representative_day[represent]['wind_speed']) #Wind Speed m/s in Slat Lake City from AMY file for 8760 hr in 2019
    ...
    def represent_day_max(self,output_prefix):
        ...
        for represent in range(self.num_clusters):
            representative_day_max[represent] = pd.read_csv(os.path.join(self.representative_days_path,output_prefix+'Represent_days_modified_'+str(represent)+'.csv'))
            ...
            G_T = list(representative_day_max[represent]['gti']) #Global Tilted Irradiation (Wh/m^2) in Slat Lake City from TMY3 file for 8760 hr a year on a titled surface 35 deg
            V_wind = list(representative_day_max[represent]['wind_speed']) #Wind Speed m/s in Slat Lake City from AMY file for 8760 hr in 2019
...
def results_extraction(problem, algorithm,path_test,output_prefix):
    components_path = os.path.join(path_test,'Energy Components')
    ...
    file_name = output_prefix+city+'_Discrete_EF_'+str(float(editable_data['renewable percentage']) )+'_design_'+str(editable_data['num_iterations'])+'_'+str(editable_data['population_size'])+'_'+str(editable_data['num_processors'])+'_processors'
    results_path = os.path.join(path_test,results_folder, file_name)
    ...
    for s in algorithm.result:
        ...
        'Cost ($)':s.objectives[0],
        'Emission (kg CO2)':s.objectives[1]}
        cost_results = problem.operating_cost(solar_results[s],wind_results[s],CHP_results[s],boiler_results[s],battery_results[s],output_prefix)
