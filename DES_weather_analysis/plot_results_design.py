...
cmap = plt.cm.RdYlGn
def ParetoFront_EFs(path_test,output_prefix):
    scatter_data = {}
    ...
    scatter_data_modified = scatter_data
    label_data = pd.read_csv(os.path.join(results_path , 'sizing_all.csv'))
    cost = [i/10**6 for i in scatter_data_modified['Cost ($)']]
    emissions = [j/10**6 for j in scatter_data_modified['Emission (kg CO2)']]
    fig,ax = plt.subplots()
    c = 'tab:red'
    m = "o"
    ...
    plt.savefig(os.path.join(results_path ,'ParetoFront_design.png'),dpi=300,facecolor='w')

ParetoFront_EFs(path_test)
...
def parallel_plots(type_plot,path_test,output_prefix):
    scatter_data = {}
    scatter_data_modified={}
    label_data = {}
    cost = {}
